...
def main1():
    ...
    try:
        # 初始位置（0）
        print("移动到初始位置")
        arm_control.move_to_position(0)
        time.sleep(2)  # 延时2秒，确保机械臂到达初始位置

        # 移动到棋子A1位置
        print("移动到棋子A1位置")
        arm_control.move_to_position('A1')
        time.sleep(2)  # 延时2秒，确保机械臂到达棋子A1位置
        ...
        # 移动到位置5
        print("移动到位置5")
        arm_control.move_to_position(5)
        time.sleep(2)  # 延时2秒，确保机械臂到达位置5

        # 关闭继电器
        print("关闭继电器")
        relay_controller.turn_off()
        time.sleep(1)  # 延时1秒，确保继电器关闭
...
def main6():
    ...
    print("按 'A' 键依次移动到序列位置 11-14。按 '1-9' 键移动到 ArmControl 的位置。")
...
def main10():
    ...
    i = 0  # 初始化图片计数
    capturing = False  # 状态标志，表示是否正在等待按键捕捉

    def on_press(key):
        nonlocal capturing, i
        try:
            if key.char == 'v' and not capturing:
                capturing = True
                print(f"正在拍摄第 {i} 张图片...")
                # 调取摄像头并拍摄图片
                ...
                if ret:
                    ...
                    # 处理拍摄的图片
                    process_image(image_path, arm, relay_controller, i)
    ...
    def process_image(image_path, arm, relay_controller, count):
        if count == 0:
            # 拍摄第 0 张照片
            board = main7(image_path)
            if board == [[0, 0, 0], [0, 1, 0], [0, 0, 0]]:
                print("调用 move 函数")
                arm.move_to_position('A2')
                execute_relay_actions(arm, relay_controller, 1)
            else:
                print("调用 re_move 函数")
                arm.move_to_position('A2')
                execute_relay_actions(arm, relay_controller, 5)
        else:
            move = main9(image_path)
            print(f"白棋下一步应该走的位置: {move}")
            ...
            print(f"棋子位置编号: {number}, 颜色: {color}")
            target_position = 32 + count  # 22, 23, 24, 25
    ...
    def execute_relay_actions(arm, relay_controller, position):
        time.sleep(2)
        relay_controller.turn_on()
        time.sleep(2)
